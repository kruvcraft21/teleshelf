...
class RedisWrapper:
    ...
    async def try_create_session(self, position: Position, tg_file_id: str) -> str:
        session_id = await self.redis.get(f"index:{position.user_id}:{position.file_id}")
        ...
        await self.redis.expire(session_id, 3600)
        await self.redis.expire(f"index:{position.user_id}:{position.file_id}", 3600)
        return session_id

    async def get_page(self, session_id: str) -> int:
        page = await self.redis.hget(session_id, "page")
        if page is None:
